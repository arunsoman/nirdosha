...
fn hashes_a_known_string_to_the_known_digest() {
    ...
    "#;
    match run(src) {
        Ok(Value::Str(s)) => assert_eq!(&*s, "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"),
...
fn two_arg_form_hashes_both_parts_without_needing_concatenation() {
    // Nirdosha `str` has no `+`/concatenation (LANGUAGE.md §2), so a
    ...
    let src = r#"
        fn chain(prev_hash: str, payload: str) -> str {
            return sha256_hex(prev_hash, payload)
        }
        fn main() -> bool {
            let genesis: str = sha256_hex("genesis")
            let h1: str = chain(genesis, "entry one")
            let h1_again: str = chain(genesis, "entry one")
            let h1_tampered_payload: str = chain(genesis, "entry ONE")
            let h1_tampered_prev: str = chain(sha256_hex("different genesis"), "entry one")
