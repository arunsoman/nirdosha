//! Tests for `sandbox`/`stop` (SANDBOXING.md's "layer 1": an affine
...
fn stopping_a_still_running_sandbox_kills_it_and_returns_negative_one() {
    ...
    // of -1. See PHASE0.md's "Thirteenth update" for the same bug caught
...
fn dropping_a_sandbox_handle_without_stopping_it_still_kills_the_process() {
    // The actual point of SANDBOXING.md's layer 1: a `sandbox` handle
