...
`compiler/src/` and `GRAMMAR.md`'s "Deliberate omissions" section confirm
it), and repeating an "already exists" claim about `capability.rs` is
exactly the mistake `goal.md`'s own honest-correction preamble already
walked back once for `capability.rs`/`ledger.rs`. This version is checked
against the actual `compiler/src/` tree, `LANGUAGE.md`, and `GRAMMAR.md`
...
`PROTOLANG_PORT.md` (the ProtoLang porting exercise) traced three "Blocked"
...
no generics (`LANGUAGE.md` §6: "No structs, enums, tuples, or generics";
`GRAMMAR.md`'s "Deliberate omissions": "No general structs/enums yet").
...
**Row 11** — a real addition to `goal.md`'s ten-row table, not a rename of
work already implied by an existing row — scoped to exactly the three type
formers that unlock those seven items, built against what `compiler/src/`
actually is today.

**The honest caveat, unchanged from last time:** Row 11 makes Nirdosha
**expressive enough** to write the programs `PROTOLANG_PORT.md` described.
...
real programs), which `goal.md` §9 already names as the standing gap this
doesn't close.
...
| Claim | Real? | Where |
|---|---|---|
| Types are `i8`…`i64`, `u8`…`usize`, `f64`, `bool`, `unit`, `str`, `box T`, `thread T`, `sandbox`, `chan T`, `tcp`, `tcp_listener`, `Vector(T,N)`, `Matrix(T,R,C)` | Yes | `ast.rs`'s `Ty` enum, `LANGUAGE.md` §2 |
| Ownership/move-checking (`box`/`thread`/`sandbox`/`tcp`/`tcp_listener` are affine) | Yes | `ownership.rs`, `LANGUAGE.md` §6 |
| Two independent bounds provers, Tier-1/2/3 escape valve | Yes | `refine.rs`, `smt.rs`, `LANGUAGE.md` §8, `goal.md` §4 |
| Reference capabilities (`iso`/`val`/`ref`/`box`/`tag`, Pony-style) | **No** | Not in `ast.rs`. Race-freedom comes entirely from `ownership.rs`'s affine move-checking (`LANGUAGE.md` §7). |
| `src/capability.rs`, `src/ledger.rs` | **No** | `goal.md`'s own honest-correction preamble already says so. |
| Koka-style effects tracked in signatures | **Not yet** | Designed, not built — `PROTOLANG_PORT.md`'s "Locked design 1." |
| `record`/`union`/`generic` keywords already exist | **No** | `GRAMMAR.md`: "No general structs/enums yet." |
| Field access (`expr.ident`), `match`/pattern matching | **No** | `GRAMMAR.md`'s `assignment` production note: "no field access... yet"; no `match` production anywhere in the grammar. Both are new work this amendment has to bring, not details to gloss over. |
| Traits / typeclasses / interfaces | **No** | Not mentioned anywhere in `LANGUAGE.md` or `GRAMMAR.md`. Generic constraints in this design are unconstrained type parameters, checked ad-hoc at use (see §3.3) — the same way `Vector(T,N)`'s `T` already works (`LANGUAGE.md` §2: "every dense linear-algebra builtin requires `T = f64` specifically"), not a typeclass system. |
| `where`-clause refinement syntax on user declarations | **No** | Refinement checking in Nirdosha today is automatic range-proving at existing boundaries (`let`/`return`/assign — `LANGUAGE.md` §2), never a type-level annotation a program writes. Row 11 extends the boundary set (§3.5), it does not add `where` syntax. |

---

## 2. Row 11: formal proposal

### 2.1 Addition to `goal.md` §1's table
...
- **No HKTs, no GADTs.** Not needed for `Option`/`Result`/`List`/`Map` or any item on `PROTOLANG_PORT.md`'s unlock list.
...
Extends `GRAMMAR.md`'s `type` production (currently a closed list of
...
"a type name applied to arguments" (`GRAMMAR.md`'s `type` production).
...
otherwise LL(1) grammar (`GRAMMAR.md`'s row-7 claim) — Rust's own
...
(`LANGUAGE.md` §6); a struct's constructor is registered under the struct's
...
amendment adds, since `GRAMMAR.md` is explicit that no field access exists
today:
...
different `Ty` values (`LANGUAGE.md` §2). A generic `struct`/`enum`
...
`LANGUAGE.md` §10 already tells for `Vector`/`Matrix`.
...
`LANGUAGE.md` §2 already says `Vector(T,N)`'s dense-linear-algebra builtins
...
production anywhere in `GRAMMAR.md`). Sum types are useless without one, so
...
- **Refinement bounds** (`refine.rs`/`smt.rs`): no new `where` syntax (§1's
  ...
  the existing boundary set — `let`/`return`/assign (`LANGUAGE.md` §2) — is
...
Same discipline as `TRANSACT.md` and `SANDBOXING.md` — one proven layer
before the next:
...
1. **Shipped.** `struct`/`enum` grammar + parser, fixed concrete fields, no
   ...
   order `TRANSACT.md` and the effects design in `PROTOLANG_PORT.md` both
   used — see `GRAMMAR.md`'s `struct_decl`/`enum_decl`/`match_expr`
...
2. **Shipped.** `typeck.rs`: declaration registration (plus two
   ...
   existing name-resolution table functions already use (`LANGUAGE.md`
...
question to pick up once Row 11 ships, the same way `PROTOLANG_PORT.md`
...
`PROTOLANG_PORT.md`, traced there to this exact gap. None of them needed
...
*Amendment to Nirdosha design brief. References: `goal.md`, `LANGUAGE.md`,
`GRAMMAR.md`, `PROTOLANG_PORT.md`, `compiler/src/{ast,typeck,ownership,
refine,smt}.rs`.*
