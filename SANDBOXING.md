...
Rows 1–3 of `goal.md` are all versions of the same claim: the type system,
...
1. **Done — `sandbox`/`stop` (PHASE0.md's "Thirteenth update").** An
   ...
   this interpreter later, not just a test-only workaround. See PHASE0.md
...
- **A sandbox gets its own structured error family, not a reuse of
  ...
  already documented for `chan T` generally (see `PHASE0.md`'s row 3
...
- **No bespoke "spawns a sandbox" effect marker before the real effects
  system.** `spawn`/`chan` could ship years ahead of goal.md's own phase
  ...
  plainly it isn't" (see every `PHASE0.md` row-status entry). Treat "can
